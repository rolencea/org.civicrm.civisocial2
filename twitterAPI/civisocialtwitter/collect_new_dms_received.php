...
if (mysqli_num_rows($results)) {
	print 'Old received DMs must be collected first.';
	exit;
}

$query = "SELECT max(dm_id) AS since_id
	FROM dms
	WHERE recipient_user_id = $engagement_user_id";
$results = $db->select($query);
$row = mysqli_fetch_assoc($results);
...
$db->update('engagement_account','new_dms_received_collected=now()',"user_id=$engagement_user_id");
?>
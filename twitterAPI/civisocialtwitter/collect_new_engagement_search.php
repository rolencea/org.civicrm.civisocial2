...
$query = "SELECT search_since_id
	FROM engagement_account
	WHERE old_search_collected != '0000-00-00'";
$results = $db->select($query);
if (mysqli_num_rows($results)==0) {
	print 'Old engagement search must be collected first.';
	exit;
}

$row = mysqli_fetch_assoc($results);
$since_id = $row['search_since_id'];

while (true) {

	$connection->request('GET', $connection->url('1.1/search/tweets'), 
      array('q' => $engagement_screen_name,
        'result_type' => 'recent',
        'lang' => 'en',
...
$db->update('engagement_account',"new_search_collected=now(),search_since_id=$since_id",
